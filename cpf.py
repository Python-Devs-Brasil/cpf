...
class CPF(object):
    ...
    def __init__(self, cpf=''):
        ...
        if not len(self.__nums__) in (0, 9):
            raise CPFError(
                "O CPF tem %d dígitos em vez de 9" %
                len(self.__nums__)
            )

        if not len(self.__validation__) in (0, 2):
            raise CPFError(
                "O dígito de verificação tem %d dígitos em vez de 2" %
                len(self.__validation__)
            )
    ...
    def __str__(self, usedots=True):
        ...
        >>> print c
        123.456.789-00
        >>> print c.__str__(False)
        123456789-00
        """

        if usedots:
            masc = '%d%d%d.%d%d%d.%d%d%d-%d%d'
        else:
            masc = '%d%d%d%d%d%d%d%d%d-%d%d'

        return masc % tuple(self.__nums__ + self.__validation__)
